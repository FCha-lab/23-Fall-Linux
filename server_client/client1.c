...
int main(){
	...
	fd1 = open("./client1fifo1", O_RDONLY);
	fd2 = open("./client1fifo2", O_WRONLY);
...
void* TaskCode1(void* argument) {
	...
	while (1) {
		...
		printf("[%02d:%02d]Client 2 -> %s",timeInfo->tm_hour, timeInfo->tm_min, receiveMsg);
	}
